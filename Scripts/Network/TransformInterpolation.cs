...
namespace Multiplayer.Smartfox.Network
{
    ...
    public class TransformInterpolation : MonoBehaviour
    {
        ...
        private TransformHandler[] bufferedStates = new TransformHandler[20];
        ...
        public void ReceivedTransform(TransformHandler ntransform)
        {
            if (!running)
            {
                return;
            ...
            if (mode == InterpolationMode.INTERPOLATION && bufferedStates[0].TimeStamp > interpolationTime)
            {
                for (int i = 0; i < statesCount; i++)
                {
                    ...
                    if (bufferedStates[i].TimeStamp <= interpolationTime || i == statesCount - 1)
                    {
                        ...
                        TransformHandler rhs = bufferedStates[Mathf.Max( i-1, 0 )];
                        // The best playback state (closest to 100 ms old (default time))
                        TransformHandler lhs = bufferedStates[i];
        ...
        private void UpdateValues()
        {
            ...
            interpolationBackTime = ping < 50 ? 50 : ping < 100 ? 100 : ping < 200 ? 200 : ping < 400 ? 400 : ping < 600 ? 600 : 1000;
        }
    }
}